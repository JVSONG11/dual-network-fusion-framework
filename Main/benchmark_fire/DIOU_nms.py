...
from ensemble_boxes.diou_nms_module import diou_nms
import ipdb
...
def process_single_id(id, res_boxes, weights, params):
    ...
    if run_type == 'diou-nms':
        iou_thr = params['iou_thr']
        merged_boxes, merged_scores, merged_labels = diou_nms(
            boxes_list, scores_list, labels_list,
            weights=weights, iou_thr=iou_thr
        )
    else:
        raise ValueError("Unsupported run_type: {}".format(run_type))
...
def ensemble(benchmark_csv, weights, params, get_score_init=True):
    ...
    ensemble_preds.to_csv("diou_nms.csv", index=False)
    get_coco_score("diou_nms.csv")
