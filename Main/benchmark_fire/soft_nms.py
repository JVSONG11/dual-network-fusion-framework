...
def get_coco_annotations_data():
    ...
    with open(file_in) as json_file:
        ...
        for i in range(len(data['images'])):  # 能正确读取数据
            ...
            images[image_id] = data['images'][
                i]  # 能正确获取单张图片的信息 图片信息{'file_name': '00006.jpg', 'height': 463, 'width': 670, 'id': '00006'}
...
def ensemble_predictions(pred_filenames, weights, params):
    ...
    for i in range(procs_to_use):
        ...
        p = Process(target=process_part_of_data,
                    args=(i, return_dict, ids_to_use[start:end], res_boxes, weights, params))  # i=0;
        jobs.append(p)
        p.start()
    ...
    num_images = len(ids_to_use)
    fps = num_images / total_time
    print(f'Run time: {total_time:.2f} seconds')
    print(f'Processed {num_images} images -> FPS: {fps:.2f}')
    return res
...
def ensemble(benchmark_csv, weights, params, get_score_init=True):
    ...
    ensemble_preds.to_csv("soft_nms.csv", index=False)
    get_coco_score("soft_nms.csv")
