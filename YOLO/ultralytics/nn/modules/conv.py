...
def autopad(k, p=None, d=1):  # kernel卷积核, padding填充, dilation扩张率
...
class Conv(nn.Module):
    ...
    default_act = nn.SiLU()  # default activation 默认的激活函数
    ...
    def forward_fuse(self, x):  # 用于Model类的fuse函数融合Conv+BN加速推理，一般用于测试/验证阶段
...
class DWConv(Conv):  # 深度可分离卷积，继承自Conv
    """Depth-wise convolution."""

    def __init__(self, c1, c2, k=1, s=1, d=1, act=True):  # ch_in, ch_out, kernel, stride, dilation, activation
        """Initialize Depth-wise convolution with given parameters."""
        super().__init__(c1, c2, k, s, g=math.gcd(c1, c2), d=d, act=act)
        # g=math.gcd(c1, c2) 分组数是输入通道（c1）和输出通道（c2）的最大公约数


class DWConvTranspose2d(nn.ConvTranspose2d):  # 带有深度分离的转置卷积，继承自nn.ConvTranspose2d
...
class Concat(nn.Module):  # 当dimension=1时，将多张相同尺寸的图像在通道维度维度上进行拼接
