...
class SSD(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        # 定义保存图像的文件夹
        # ipdb.set_trace()
        output_folder = "E:/0909/ssd/runs"
        os.makedirs(output_folder, exist_ok=True)

        # 打开/创建ssd1.txt文件用于保存predicted_class和score
        ssd1_filename = os.path.join(output_folder, "ssd_1.txt")  # 类别和得分
        with open(ssd1_filename, 'a') as ssd1_file:
            ...
            # ipdb.set_trace()
            # 保存predicted_class和score到ssd1.txt文件中
            original_image_name = os.path.basename(image.filename)
            ssd1_file.write(f"{original_image_name[0:5]},{predicted_class},{score}\n")
            ...
            draw.text(text_origin, str(label, 'UTF-8'), fill=(255, 255, 255), font=font)  # 字体
            del draw
            return image

        # 获取原始图像名称
        # ipdb.set_trace()
        image_filename = os.path.join(output_folder, original_image_name)

        # 保存图片到文件夹
        image.save(image_filename)

        print("图片已保存到", image_filename)
        print("预测结果已保存到", ssd1_filename)
