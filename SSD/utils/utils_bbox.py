        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    # ssd_name = "ssd_2.txt"  # xyxy坐标
                    # with open(ssd_name, 'a') as file_object:
                    #     good_boxes_cpu = good_boxes.cpu()
                    #     confs_cpu = confs.cpu()
                    #     labels_cpu = labels.cpu()
                    #
                    #     good_boxes_list = good_boxes_cpu.tolist()
                    #     confs_value = confs_cpu[0].item()
                    #     labels_value = labels_cpu[0].item()
                    #
                    #     output_ssd = f"{(', '.join(map(str, good_boxes_list[0])))}"
                    #     # output_ssd_xywh = f"{box_xy.flatten()[0]}, {box_xy.flatten()[1]}, {box_wh.flatten()[0]}, {box_wh.flatten()[1]}"
                    #     file_object.write(output_ssd + "\n")
                    #     print("预测结果已保存到ssd_2")
            ...
            if len(results[-1]) > 0:
                ...
                with open(ssd_name, 'a') as file_object:
                    xy = box_xy
                    wh = box_wh
                    output_ssd = f"{xy[0, 0]},{xy[0, 1]},{wh[0, 0]},{wh[0, 1]}"
                    # output_ssd_xywh = f"{box_xy.flatten()[0]}, {box_xy.flatten()[1]}, {box_wh.flatten()[0]}, {box_wh.flatten()[1]}"
                    file_object.write(output_ssd + "\n")
                    print("预测结果已保存到ssd_2")
